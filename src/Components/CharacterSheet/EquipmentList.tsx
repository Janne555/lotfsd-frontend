...
function EquipmentList({ characterName, equipment }: Props) {
  ...
  return (
    <ModuleContainer onAddClick={handleAddClick}>
        ...
        <tbody>
          {
            equipment.map(({ name, listItemId, amount, stackSize }) => (
              <tr key={listItemId ?? undefined}>
                <td>{name}</td>
                <td>{stackText(stackSize ?? 0, amount ?? 0)}</td>
              </tr>
            ))
          }
        </tbody>
      ...
      <table className={classes.table}>
        ...
        <tbody>
          {
            weapons.map(({ name, listItemId, amount, stackSize, damage, attackBonus, range, description }) => (
              <tr key={listItemId}>
                <td>{name}</td>
                <td>{attackBonus}</td>
                <td>{damage}</td>
